...
/* Typewriter pieces. The hidden full-text placeholder reserves the final
   size (so nothing reflows as text types in); the overlay shows the typed
   slice on top. */
export const TypedOverlay = styled.span`
  position: absolute;
  left: 0;
  top: 0;
  right: 0;
...
export const Meta = styled.div`
  ...
  border-radius: 8px;
  position: relative;
  animation: ${dvFadeUp} 0.5s 2.2s both ease-out;
  ${noMotion}
...
export const SectionNum = styled.span`
  ...
  color: ${dim2};
  font-size: 12px;
  animation: ${dvFadeIn} 0.4s 3.05s both ease-out;
  ${noMotion}
`;
...
export const SectionTitle = styled.span`
  ...
  animation: ${dvFadeIn} 0.4s 3.1s both ease-out;
  ${noMotion}
`;
...
export const SectionLine = styled.span`
  ...
  animation: ${dvDrawLine} 0.7s 2.95s both cubic-bezier(0.4, 0, 0.2, 1);
  ${noMotion}
`;
...
export const SectionCount = styled.span`
  ...
  animation: ${dvFadeIn} 0.4s 3.2s both ease-out;
  ${noMotion}
`;

/* ---------- cards ---------- */
/* Wraps the grid so the scan line can overlay it without being a grid child
   (which would shift the cards' nth-child parity and break the
   last-card-spans-two-columns rule). */
...
/* "Scanner" sweep: an accent line passes top→bottom over the card grid as the
   cards reveal. Absolutely positioned so it doesn't occupy a grid cell. */
export const GridScan = styled.div`
  position: absolute;
  left: -6px;
  right: -6px;
  top: 0;
  ...
  box-shadow: 0 0 14px 2px rgba(31, 79, 214, 0.35);
  animation: ${gridScanSweep} 1.9s 3.2s cubic-bezier(0.4, 0, 0.2, 1) both;
...
export const Kind = styled.span`
  ...
  /* Animation is disabled here, so reveal the (informative) badge directly. */
  @media (prefers-reduced-motion: reduce) {
    opacity: 1;
  }
...
export const Foot = styled.div`
  ...
  color: ${dim2};
  letter-spacing: 0.08em;
  animation: ${dvFadeIn} 0.6s 5.0s both ease-out;
  ${noMotion}
`;
